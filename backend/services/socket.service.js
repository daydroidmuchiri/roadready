...
function createSocketService({ server, clientOrigin, jwtSecret, Users, Jobs }) {
...
function initSocket(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('update_location', async ({ location }) => {
      try {
        ...
        if (activeJob?.motoristId) {
          emitToJob(activeJob.id, activeJob.motoristId, socket.user.id, 'provider_location', {
    ...
    socket.on('disconnect', reason => console.log(JSON.stringify({
      level: 'INFO',
  ...
  return { io, emitToJob, emitToUser, emitToAdmins };
}
